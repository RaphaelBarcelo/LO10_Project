...
def get_champions_trends():
    ...
    for champion in current_champ_list['data']:
        all_champions_popularity_comparisons[champion] = {}
    for champion_1 in current_champ_list['data']:
        champion1_popularity_comparison = {}
        for champion_2 in current_champ_list['data']:
            if champion_1 != champion_2:
                if champion_1 not in all_champions_popularity_comparisons[champion_2]:
                    ...
                    champion1_popularity_comparison[champion_2] = random.randint(0, 100)
                    all_champions_popularity_comparisons[champion_2][champion_1] = random.randint(0, 100)

                    #pytrends1.build_payload(search_terms, geo='US', timeframe="today 12-m")
                    #df1 = pytrends1.interest_over_time()
                    #champion1_popularity_comparison[champion_2] = df1[search_terms[1]].mean().round(0)
                    #all_champions_popularity_comparisons[champion_2][champion_1] = df1[search_terms[0]].mean().round(0)
                    #time.sleep(1.1)
